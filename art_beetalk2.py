...
def grade_flowers(flower_pop,best_critics):
 for crit in best_critics:
   ...
   for k in range(len(flower_pop)):
    ...
    # art.fitness+=k
    #else:
    # art.fitness-=(k/len(flower_pop))
 results_dict=dict()
 for k in flower_pop:
  k.raw_fitness=k.fitness
  if(not k.nectar in results_dict):
   results_dict[k.nectar]=k.fitness
  else:
   results_dict[k.nectar]=max((results_dict[k.nectar],k.fitness))
 print results_dict
...
def grade_critics(critic_pops,best_art):
 ...
 # b = random.choice(best_art)
 # flower_archive.append(random.choice(best_art))
...
while(True):
 ...
 if(render):
  ...
  for k in range(min(16,len(flower_pops))):
   ind = flower_best[k]
   render_picture(ind.nectar,25+(k%gs)*180,25+(k/gs)*180,PXS,ind.get_picture())
  screen.blit(background,(0,0))
  pygame.display.flip()
