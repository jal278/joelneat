...
public:
	...
	*/
	...
	void evaluate_individual(individual* ind,population* pop,bool fitness=true);

	//maintain list of fittest organisms so far
	void update_fittest(individual* org)
	{
		int allowed_size=5;
	...
	void serialize_fittest(char *fn)
	{
		...
		for(int i=0;i<(int)fittest.size();i++)
			newpop.members.push_back(fittest[i]->ind);
	...
	vector<sort_pair> map_novelty_pop(float (*nov_func)(noveltyitem*,noveltyitem*),noveltyitem* newitem, population* pop)
	{
		...
		for(int i=0;i<(int)pop->members.size();i++)
		{
			novelties.push_back(make_pair((*novelty_metric)(pop->members[i]->noveltypoint,newitem),
				pop->members[i]->noveltypoint));
		}
		return novelties;
	}
	...
	bool Serialize(char* fname)
	{
		population newpop;
		for(int i=0;i<(int)novel_items.size();i++)
			newpop.members.push_back(novel_items[i]->ind);
		newpop.save(fname);
		return true;
	}
